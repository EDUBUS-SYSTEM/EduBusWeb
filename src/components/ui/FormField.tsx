    ...
    <div className={`space-y-2 ${className}`}>
      ...
      <div className="relative">
        {React.cloneElement(children as React.ReactElement<Record<string, unknown>>, {
          className: `w-full px-4 py-3 border-2 rounded-lg transition-all bg-gray-50 focus:bg-white resize-none ${hasError
...
export function CharacterCounter({ current, max, className = "" }: CharacterCounterProps) {
  ...
  return (
    <div className={`flex items-center justify-between text-xs ${className}`}>
      <div className={`font-medium ${isOverLimit ? 'text-red-600' : isNearLimit ? 'text-orange-600' : 'text-gray-500'
        }`}>
      ...
      <div className="w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden ml-2">
        <div
          className={`h-full transition-all duration-300 ${isOverLimit ? 'bg-red-500' : isNearLimit ? 'bg-orange-500' : 'bg-green-500'
            }`}
