...
import { FaEdit, FaTrash, FaEye, FaRoute, FaClock, FaCalendarAlt, FaBus } from "react-icons/fa";
import { scheduleService, RouteSchedule, RouteScheduleQueryParams } from "@/services/api/scheduleService";
...
export default function RouteScheduleList({ searchTerm, filterActive, refreshTrigger }: RouteScheduleListProps) {
  ...
  const filteredRouteSchedules = routeSchedules.filter(routeSchedule => {
    const matchesSearch = routeSchedule.routeName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         routeSchedule.scheduleName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         routeSchedule.notes?.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesFilter = filterActive === null || routeSchedule.isActive === filterActive;
  ...
  const handleDeleteRouteSchedule = async (routeScheduleId: string) => {
    ...
    try {
      ...
      const errorMessage = getErrorMessage(error, "Failed to delete route schedule. Please try again.");
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-sm text-gray-600">
          {filteredRouteSchedules.length} of {routeSchedules.length} route schedules
        </div>
      </div>
      ...
      {filteredRouteSchedules.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">No route schedules found</h3>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-gray-700 font-medium">{routeSchedule.scheduleName}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center gap-2 text-gray-600">
                      ...
                      <span className="text-sm">
                        {formatDate(routeSchedule.effectiveFrom)} - {formatDate(routeSchedule.effectiveTo)}
                      </span>
                    </div>
      ...
      {showDetails && selectedRouteSchedule && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-gray-800">Route Schedule Details</h3>
                <button
              ...
              <div className="space-y-6">
                <div>
                  <h4 className="text-lg font-semibold text-gray-800 mb-2">{selectedRouteSchedule.routeName}</h4>
                  <div className="flex items-center gap-2 mb-2">
                    <FaClock className="w-4 h-4 text-green-500" />
                    <span className="text-gray-700 font-medium">{selectedRouteSchedule.scheduleName}</span>
                  </div>
                  {selectedRouteSchedule.notes && (
                    <p className="text-gray-600">{selectedRouteSchedule.notes}</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-gray-900 font-mono text-sm">{selectedRouteSchedule.routeId}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Schedule ID</label>
                      <p className="text-gray-900 font-mono text-sm">{selectedRouteSchedule.scheduleId}</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-gray-900">
                        {formatDate(selectedRouteSchedule.effectiveFrom)} - {formatDate(selectedRouteSchedule.effectiveTo)}
                      </p>
                    </div>
                    <div>
