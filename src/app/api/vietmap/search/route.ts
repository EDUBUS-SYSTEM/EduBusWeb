...
export async function GET(request: NextRequest) {
  try {
    ...
    const vietmapUrl = `https://maps.vietmap.vn/api/search/v4?${params}`;
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`[VietMap Proxy] Search API returned ${response.status}`);
        return NextResponse.json(
      ...
      console.log('[VietMap Proxy] Successfully fetched search results');
