...
export default function RequestDetailModal({ request, onClose }: RequestDetailModalProps) {
  ...
  // Load current unit price if request doesn't have it
  useEffect(() => {
    const loadCurrentUnitPrice = async () => {
      if (!request.unitPricePerKm || request.unitPricePerKm === 0) {
        try {
          setIsLoadingPrice(true);
          const unitPrice = await unitPriceService.getCurrentEffectiveUnitPrice();
          setCurrentUnitPrice(unitPrice.pricePerKm);
        } catch (error) {
          console.warn('Failed to load current unit price:', error);
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-yellow-50 rounded-xl p-6 border border-yellow-200">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-800 mt-1">
                  {isLoadingPrice ? (
                    ...
                    formatCurrency(request.unitPricePerKm || currentUnitPrice)
                  )}
                </p>
                {(!request.unitPricePerKm || request.unitPricePerKm === 0) && currentUnitPrice && (
                  <p className="text-xs text-gray-500 mt-1">
                    💡 Using current unit price from system
                  </p>
                )}
              </div>
              
              <div>
                <label className="text-sm font-medium text-gray-700">Total Semester Price</label>
                <p className="text-2xl font-bold text-yellow-800 mt-1">
            ...
            <div className="mt-6 pt-6 border-t border-yellow-200">
              <h4 className="text-md font-semibold text-yellow-900 mb-3">Semester Details</h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-700">Semester</label>
                  <p className="text-gray-900 mt-1">
                    {request.semesterName || 'N/A'} {request.academicYear || 'N/A'}
                  </p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">School Days</label>
                  <p className="text-gray-900 mt-1">
                ...
                <div>
                  ...
                  <p className="text-gray-900 mt-1">
                    {(request.distanceKm * (request.totalSchoolDays || 1)).toFixed(1)} km
                  </p>
                </div>
              </div>
            </div>
