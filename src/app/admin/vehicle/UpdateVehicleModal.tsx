...
export default function UpdateVehicleModal({
  ...
  const [formData, setFormData] = useState<UpdateVehicleRequest>({
    ...
    status: mapStatusToNumber(vehicle.status),
  });
  ...
  function mapStatusToNumber(status: VehicleDto["status"]): number {
  ...
  const handleInputChange = (
    field: keyof UpdateVehicleRequest,
    value: string | number
  ) => {
      ...
      <div className="w-full max-w-lg rounded-2xl bg-[#fefce8] shadow-xl">
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          ...
          <button
            ...
            aria-label="Close update vehicle modal"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
